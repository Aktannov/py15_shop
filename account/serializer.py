...
class ChangePasswordSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        ...
        if pass1 != pass2:
            raise serializers.ValidationError('Параоли не совпадают')
        return attrs
...
#     user.save()
#     send_mail('Востоновление пароля',
...
class ForgotPasswordSerializer(serializers.Serializer):
    ...
    def send_code(self):
        ...
        send_mail(
            ...
            f'Ваш код пдтверждения: {user.activation_code}',
            'test@gamil.com'
        )
...
class DeiSerializer(serializers.Serializer):
    email = serializers.EmailField(required=True)
    def validate_email(self, email):
        if not User.objects.filter(email=email).exists():
            raise serializers.ValidationError('придурок ты не зарег')
        return email
